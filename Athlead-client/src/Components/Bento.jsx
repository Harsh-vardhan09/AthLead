  ...
  if (isLoading) return (
    <BentoGrid className="w-4xl mx-auto my-6">
      {Array.from({ length: 8 }).map((_, i) => (
        <Skeleton key={i} className={i === 3 || i === 6 ? "md:col-span-2" : ""} />
      ))}
    </BentoGrid>
  )
  return (
...
const Skeleton = ({ className }) => (
  <div className={cn("animate-pulse space-y-4 flex flex-1 flex-col w-full h-60 min-h-24 rounded-xl bg-linear-to-b from-neutral-200 to-neutral-100 p-8", className)}>
